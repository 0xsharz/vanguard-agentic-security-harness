...
Each finding gets a unified diff **plus a security test** that fails on the vulnerable
code and passes on the fix. Patches are generated by reading code — never executed,
never applied — so each is marked `needs_verification` until a sandbox run confirms it.
